  ...
  db.run(
    ...
    function (err) {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: 'Error al crear la actividad.', error: err.message });
      }
...
exports.getAllActivities = (req, res) => {
  db.all(`SELECT * FROM activities`, [], (err, rows) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: 'Error al obtener las actividades.', error: err.message });
    }
    res.json(rows);
  });
};
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: 'Error al actualizar la actividad.', error: err.message });
      }
      if (this.changes === 0) {
...
exports.deleteActivity = (req, res) => {
  ...
  db.run(`DELETE FROM activities WHERE id = ?`, [id], function (err) {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: 'Error al eliminar la actividad.', error: err.message });
    }
    if (this.changes === 0) {
      return res.status(404).json({ message: 'Actividad no encontrada.' });
    }
    ...
    function (err) {
      if (err) {
        ...
        return res.status(500).json({
          message: 'Error al inscribirse a la actividad o ya estás inscrito.',
          error: err.message
        });
      }
      res
        .status(201)
        .json({ message: 'Inscripción realizada con éxito.', inscriptionId: this.lastID });
    }
  );
};
  ...
  db.all(
    ...
    (err, rows) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: 'Error al obtener la lista de participantes.', error: err.message });
      }
      res.json(rows);
    }
  );
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        ...
        return res.status(500).json({
          message: 'Error al actualizar el control de participantes.',
          error: err.message
        });
      }
      if (this.changes === 0) {
